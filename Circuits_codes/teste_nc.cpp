...
int main(){
	...
	for(int i = 0; i < (int)(last_time/dt); i++){
		for(int j = 0; j < ((int)(0.25*amount_neurons)); j++) circuit->set_I_inj(j+1, 50+0*distribution(generator));
...
void create_random_circuit(Neural_Circuit<> *circuit){
	...
	float prob_excitatory = 1.8;
	int density_synapse = 2.2*amount_neurons;
	...
	for(int i = 1; i <= amount_neurons; i++){
		...
		for(int j = 1; j <= amount_neurons; j++){
			variables[0] = 70+0*((float)rand())/RAND_MAX;
